...
● ¿Cuáles son los nombres completos de todos los clientes en la base de
datos?

SELECT CONCAT(NombreCliente, ' ', ApellidoContacto) AS Nom_Completo FROM clientes;

● ¿Cuál es el límite de crédito de cada cliente?

SELECT Nom_Cliente, LimiteCredito FROM clientes;

● ¿Cuál es el nombre y apellido de contacto de los clientes en una ciudad
específica?

SELECT Nom_Contacto, Apell_Contacto 
FROM clientes
WHERE Ciudad = 'Ciudad';

● ¿Cuál es la lista de todos los empleados asociados a cada cliente?

SELECT clientes.Nom_Cliente, empleados.Nom_Empleado 
FROM clientes
LEFT JOIN empleados ON clientes.Id_Empleado = empleados.Id_Empleado;

● ¿Cuál es la cantidad de clientes en cada estado?

SELECT Estado, COUNT(*) AS Cant_Clientes 
FROM clientes
GROUP BY Estado;

● ¿Cuál es el número de teléfono de contacto de cada cliente?

SELECT Nom_Cliente, Telefono FROM clientes;

● ¿Cuál es la dirección completa de cada cliente, incluyendo dirección 1,
dirección 2, ciudad, estado, código postal y país?

SELECT Nom_Cliente, CONCAT(Direccion1, ', ', IFNULL(Direccion2, ''), ', ', Ciudad, ', ', IFNULL(Estado, ''), ', ', IFNULL(CodigoPostal, ''), ', ', Pais) AS Direccion
FROM clientes;

● ¿Cuál es el promedio de límite de crédito para todos los clientes en una
ciudad específica?

SELECT Ciudad, AVG(LimiteCredito) AS PromedioLimiteCredito 
FROM clientes
WHERE Ciudad = 'Ciudad'
GROUP BY Ciudad;

● ¿Cuál es el cliente con el límite de crédito más alto y más bajo?

SELECT 
   (SELECT Nom_Cliente FROM clientes WHERE Limite_Credito = (SELECT MAX(LimiteCredito) FROM clientes)) AS ClienteConLimiteAlto,
   (SELECT Nom_Cliente FROM clientes WHERE LimiteCredito = (SELECT MIN(LimiteCredito) FROM clientes)) AS ClienteConLimiteBajo;

● ¿Cuál es la lista de clientes cuyo nombre de contacto contiene un término
específico?

SELECT Nom_Cliente 
FROM clientes
WHERE CONCAT(Nom_Contacto, ' ', Apell_Contacto) LIKE '%Termino%';
